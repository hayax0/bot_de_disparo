...
// Health check — Readiness probe (dependências essenciais disponíveis: PostgreSQL SELECT 1, Redis PING, Worker Heartbeat)
const handleReadinessCheck = async (req: express.Request, res: express.Response) => {
  ...
  // 1. PostgreSQL (SELECT 1 leve sem escrita)
  try {
  ...
  // 2. Redis PING e Heartbeat do Worker
  try {
    ...
    if (lastHeartbeat) {
      ...
      if (process.uptime() > 60) {
        workerStatus = 'degraded: nenhum heartbeat reportado';
      } else {
